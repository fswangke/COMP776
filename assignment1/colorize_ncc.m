%COLORIZE_NCC Colorize Sergei Mikhailovich Prokudin-Gorskii pictures.
%   [colorim, offsetR, offsetB] = colorize_ncc(imname) takes accepts 
%   negative plate file name and returns aligned color images files. Alignment 
%   is done with NCC distance metric.
%
%   Ke Wang (kewang@cs.unc.edu)
function [colorim, offsetR, offsetB] = colorize_ncc(imname)
    ...
    %% Align images
    offsetR = align_ncc(subR, subG);
    offsetB = align_ncc(subB, subG);
