%COLORIZE_HIRES_SSD Colorize Sergei Mikhailovich Prokudin-Gorskii pictures.
%   [colorim, offsetR, offsetB] = colorize_hires_ssd(imname) takes accepts 
%   negative plate file name and returns aligned color images files. Alignment 
%   is done with SSD distance metric, and accelerated by Gaussian pyramids.
%
%   Ke Wang (kewang@cs.unc.edu)
function [colorim, offsetR, offsetB] = colorize_hires_ssd(imname)
    ...
    offsetR = pyramid_align_ssd(subR, subG);
    offsetB = pyramid_align_ssd(subB, subG);
